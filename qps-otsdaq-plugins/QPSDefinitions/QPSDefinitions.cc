...
// TODO: Get this from theXDAQContextConfigTree
float scale = 7.0;
...
void qps_parse_from_raw(qps_sample* qps_data_ptr, uint64_t* word_ptr)
{
	...
	data >>= 6;                                // Remove first 6 bits (all zeros)
	uint32_t data_sext   = data | (data & 0x20000 ? 0xFFFC0000 : 0);  // Sign extend
	int32_t  data_signed = static_cast<int32_t>(data_sext);           // Signed conversion

	// Convert to physical voltage

	qps_data_ptr->data = 2.0 * scale * ((float)(data_signed) / pow(2.0, 18.0));
